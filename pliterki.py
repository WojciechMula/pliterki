...
3. polski słownik do aspella; polecam <http://www.kurnik.pl/slownik>;
   jeśli używasz Debiana: apt-get install aspell-pl
...
słowa; jeśli nie będzie ono należało do słownika zostaniemy ostrzeżeni. Po
zatwierdzeniu, R spowoduje zamianę podświetlanego słowa, natomiast A zamianę
tego i wszystkich następnych.

Polecenia I spowoduje, że zaznaczone słowo zostanie uznane za poprawne
i więcej nie będziemy nękani pytaniami o jego pisownię.

Polecenie A <numer> (spacja nie jest wymagana) jest szczególnie pożyteczne
jeśli widzimy, że na liście znajdują się słowa, które na pewno w
przetwarzanym tekście nie wystąpią (np. dla "lub" lista propozycji to:
"łub" i "lub" -- to pierwsze nie jest zbyt powszechne).

UWAGA! Słowa dodane poleceniami R i A, oraz te które zostały zignorowane
~~~~~~ poleceniem I nie są nigdzie zapisywane. Co więcej, jeśli sprawdzamy
	   wiele plików, to jesteśmy pytani czy skasować te słowa przed
	   przystąpieniem do sprawdzania następnego pliku. Można wówczas skasować,
	   pozostawić słowa i również ustawić, by program automatycznie kasował
	   lub nigdy nie kasował obu zbiorów słów.
